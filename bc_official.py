...
# my computer:
# ch_border = gpd.read_file(str(in_path) + "\switzerland\ch_bordercrossings\swiss_border\g2l19.shp")
ch_border = gpd.read_file(str(in_path) + "\switzerland\ch_bordercrossings\swiss_border\\bci_path.shp")
# europe_network = gpd.read_file(str(in_path) + "\europe\europe_network\gdf_MTP_europe.shp")
# europe_network = gpd.read_file(str(in_path) + "\switzerland\ch_network\gdf_MTP_europe.shp")
europe_network = gpd.read_file(str(in_path) + str(merged_network) + "\\gdf_MTP_europe.shp")
# europe_network = gpd.read_file(str(in_path) + "\merged_networks\ch1234\gdf_MTP_europe.shp")
# LOAD official FREIGHT DATA
official_df = pd.read_csv(str(in_path) + "\\freight_data\\freight\\official_counting_ot.csv")

# ivt computer
# ch_border = gpd.read_file(str(in_path) + "\switzerland\ch_bordercrossings\gd-b-00.03-875-gg19\ggg_2019-LV95\shp\g2l19.shp")
# ch_border = gpd.read_file(str(in_path) + "\switzerland\ch_bordercrossings\gd-b-00.03-875-gg19\ggg_2019-LV95\shp\k4l19.shp")
# ch_border = gpd.read_file(str(in_path) + "\switzerland\ch_bordercrossings\swiss_border\bci_lines.shp")
# europe_network = gpd.read_file(str(in_path) + "\europe\europe_network\gdf_MTP_europe.shp")

print('Nlines in ch_border: '+ str(len(ch_border)))
print('Nways in europe_network: '+ str(len(europe_network)))
print('Nbc in official_df: '+ str(len(official_df)))

# ch_border.crs = {"init" : "EPSG:4326"}
# ch_border = ch_border.to_crs({"init" : "EPSG:2056"})
# print(ch_border['geometry'][0])
ch_border['geometry'][0]
# ch_border['geometry'][0].exterior

# find which ways intersect the CH border
points_list = []
id_list = []
ls_count = 0
...
# loops over all the ways defined in the network and keeps the coordinates and the new_id of the way
for i in range(0, len(europe_network)):
...
crossing_points_df = pd.DataFrame.from_records(points_list, columns = [
    "geometry", "new_id"
])
crossing_points_df.to_csv(str(out_dir) + "\crossing_onlypoints.csv", sep = ",", index = None)

gdf = gpd.GeoDataFrame(crossing_points_df)
gdf.to_file(str(out_dir) + "\crossing_onlypoints.shp")


# CHECKPOINT: load file created before

# ch_bc = gdf
# ch_bc = gpd.read_file(r"C:\Users\Ion\IVT\OSM_python\switzerland\ch_bordercrossings\crossing_onlypoints_opt.shp")
# ch_bc = gpd.read_file(r"C:\Users\gaion\freight\switzerland\ch_bordercrossings\crossing_onlypoints_opt.shp")
ch_bc = gpd.read_file(str(out_dir) + "\crossing_onlypoints.shp")
official_df = pd.read_csv(str(in_path) + "\\freight_data\\freight\\official_counting_ot.csv")
...
# DROP ROWS with no data or which do not overpass a record of 5000 in any value
droprows=[]
min_freight = 0 #adjust this value for the filter minimum
for i in range(0,len(official_df)):
    if (
...
for i in range(0, len(ch_bc)):
    x = ch_bc.iloc[i]['geometry'].x
    y = ch_bc.iloc[i]['geometry'].y
    G_lonlat.append((x, y))
...
def closest_bc(bc, rowname):
    ...
    for j in range(0, len(ch_bc)):
        ...
        # Last, for each closest crossing in the network, a grouping of the crossings in a radious of 500 m is done,
...
with open(str(out_dir) + '\wayidbycp_dict' + '.pkl', 'wb') as f:
    pickle.dump(wayid_by_cp, f, pickle.HIGHEST_PROTOCOL)
print(len(wayid_by_cp))
...
bc_df.to_csv(str(out_dir) + "\\bc_df.csv", sep=",", index=None, encoding='latin1')

gdf = gpd.GeoDataFrame(bc_df)
gdf.to_file(str(out_dir) + "\\bc_df.shp", encoding='latin1')
...
G=nx.read_gpickle(str(in_path) + "\merged_networks\eu123_ch4\eu_network_largest_graph_bytime.gpickle")
print('G graph (Nnodes/Nedges): '+ str(G.number_of_nodes()) + '/' + str(G.number_of_edges()))

# # IMPORT nodes_europe
file = open(str(in_path) + "\merged_networks\eu123_ch4\europe_ways_splitted_dict.pkl",'rb')
europe_ways_splitted_dict = pickle.load(file)
file.close()
print('Nways in europe_ways_splitted_dict: '+ str(len(europe_ways_splitted_dict)))
...
print(len(all_newids))
print(len(elected_newids))
print(len(none_elected))
...
nx.write_gpickle(G, str(out_dir) + "\eu_network_graph_with_official_bc.gpickle")
print('G graph (Nnodes/Nedges): ' + str(G.number_of_nodes()) + '/' + str(G.number_of_edges()))
print('New graph has: ' + str(
    len([len(c) for c in sorted(nx.connected_components(G), key=len, reverse=True)])) + ' island with ' + str(
    G.number_of_nodes()) + ' nodes')
print('From ' + str(len(none_elected)) + ' none_elected Edges: ' + str(
    deleted_edges) + ' deleted correctly (others not in graph)')
print('New graph exported as "eu_network_graph_with_official_bc"')
